...
선행 미충족 시, 미충족 스텝이 **공유 단계**(`2.1, 2.2, 3.1, 3.2, 3.3, 5.1`)인 경우:

```
[미충족 스텝명]이 완료되지 않았습니다.
다른 작업자가 이미 완료했나요? (y/n)
```

- `n` → 일반 선행 미충족 안내로 이동
- `y` → Notion MCP로 일감번호(예: DCL-1234) 기반 해당 산출물 페이지 존재 여부 확인:
  - 존재하면 → `done = true`, `validated = true`, `artifactPageId` 기록 후 선택한 스텝 진행
  - 존재하지 않으면 → "Notion에서 확인되지 않았습니다. 해당 단계를 먼저 완료해주세요." 출력 후 종료

미충족 스텝이 공유 단계가 아닌 경우:
```
...
| 2.1  | `skills/planning/spec-review`                            | Hook 자동 |
| 2.2  | `skills/planning/spec-finalize`                          | Hook 자동 |
| 3.1  | `skills/blueprint/ui-flow`                               | Hook 자동 |
| 3.2  | `skills/blueprint/data-flow`                             | Hook 자동 |
| 3.3  | `skills/blueprint/tech-spec`                             | Hook 자동 |
...
| 5.1  | `skills/testing/qa-scenario`                             | Hook 자동 |
| 5.2  | `skills/development/bug-fix`                             | 선택, 반복 가능 |
| 5.3  | `skills/testing/task-complete`                           | 없음      |

### 사후 처리

- Notion 산출물 스텝(2.1, 2.2, 3.1, 3.2, 3.3, 5.1)의 검증은 `validate-guard.js` PostToolUse Hook이 자동 처리한다. Claude가 별도로 `skills/common/validate`를 호출하지 않는다.
