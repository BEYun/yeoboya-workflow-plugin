...
Notion MCP로 산출물을 읽는다:
1. UI 흐름도 서브페이지 (`state.stages["3.1"].artifactPageId`)
2. 데이터 흐름도 서브페이지 (`state.stages["3.2"].artifactPageId`)
3. 최종 기획서 페이지 (`state.finalSpec.pageId`)
...
Notion MCP로 작업번호 페이지 하위에 **"기술 설계"** 서브페이지를 생성한다.
...
> 출력 포맷 변경 시 해당 훅의 3.3 규칙 블록을 동기화해야 합니다.
