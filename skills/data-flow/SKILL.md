...
1단계(기획서 검토)가 완료되어야 한다. notion-writer 스킬로 "기획서 검토" 서브페이지 존재 여부를 확인한다.
...
`sequenceDiagram`으로 작성한다. 주요 데이터 흐름별로 다이어그램을 분리:
...
notion-writer 스킬을 사용하여 작업번호 페이지 하위에 **"데이터 흐름도"** 서브페이지를 생성한다:
