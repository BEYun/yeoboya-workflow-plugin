...
1. **superpowers:brainstorming** — 확정된 blueprint를 입력으로, 작성 단위(데이터 모델/리포지토리/비즈니스/UI) 수준에서 세부 설계를 재점검한다. 이 시점에 **구현에 활용할 스킬 목록도 함께 결정**한다:
   - 프로젝트 개발 스킬 탐색에서 발견된 스킬
   - superpowers 스킬 중 해당 작업에 적합한 것 (예: `subagent-driven-development`, `dispatching-parallel-agents` 등)
   - 결정된 스킬 목록은 writing-plans에 명시한다
   - brainstorming의 terminal state는 writing-plans 호출이다
2. **superpowers:writing-plans** — brainstorming에서 합의된 설계와 스킬 목록을 기반으로 작업번호별 작성 plan을 작성한다. blueprint(산출물)와 plan(실행 단위)은 서로 다른 층위다. 각 실행 단위에 사용할 스킬을 명시한다.
...
1. **공통 설계** — Notion MCP로 `state.stages["3.3"].artifactPageId` 페이지를 읽어 로드
...
프로젝트 루트에서 개발 관련 스킬이 있는지 확인한다. 탐색 순서:

1. 프로젝트 루트의 `.claude/skills/` 또는 `skills/` 디렉토리 존재 여부 확인
2. 있으면 개발(코드 작성) 관련 스킬 목록을 스캔하고 사용자에게 고지:
   ```
   프로젝트 내 개발 스킬이 발견되었습니다:
   - <스킬명>: <description>
   brainstorming 시 이 스킬들을 활용합니다.
   ```
3. 없으면 조용히 넘어간다 (사용자 알림 불필요)
